...
Domoticz.devices = function(baseURL, roomID, completion, error) {
  ...
  var url = baseURL + "type=scenes&used=true&order=Name";
  if (roomID) {
      url += "&plan=" + roomID;
  }
  var devices = [];
  baseHttpRequest.get({
	...
	console.log("this is device push: " + s.Type);
	if (s.Type != "Scene")
	        devices.push(s);
      });
  ...
  url = baseURL + "type=devices&used=true&order=Name";
  if (roomID) {
      url += "&plan=" + roomID;
  }
  baseHttpRequest.get({
...
if (accessory.IsScene){
	ttay = "type=scenes&rid=";
}else{
	ttay = "type=devices&rid=";
}
  var url = accessory.platform.apiBaseURL + ttay + accessory.idx;
  baseHttpRequest.get({
    url: url
  }, function(err, response, json) {
    if (!err && response.statusCode == 200 && json !== undefined)
...
if (accessory.IsScene){
console.log(accessory.idx);
for (var i = 0; i<json.result.length; i++){
	if(json.result[i].idx != accessory.idx){
		delete json.result[i];	
	}
}
console.log(json);
}
