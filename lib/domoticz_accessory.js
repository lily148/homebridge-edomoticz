...
if (Type == "Group")
    this.swTypeVal = 0; 
else this.swTypeVal = swTypeVal;
console.log(this.swTypeVal);
...
if (Type == "Group"){
    this.IsScene = true;
    this.swType = "On/Off";
    this.Type = "Light/Switch";

}
console.log(this.hwType);
console.log(this.Type);
console.log(this.swType);
...
eDomoticzAccessory.prototype = {
            ...
            case this.swTypeVal == Constants.DeviceTypeMedia:
                {
                    ...
                    {
                        service = this.getService(Service.Fan);
                        brightnessCharacteristic = this.getCharacteristic(service, Characteristic.RotationSpeed);
                    }else
                    {
                        service = this.getService(Service.Lightbulb);
                        brightnessCharacteristic = this.getCharacteristic(service, Characteristic.Brightness);
                    }
                    ...
                    if (isOn && this.factor) {
                        var handleBrightnessChange = function (brightness) {
                            ...
                            if (brightness > 0) {
                                setTimeout(function () {
                                    if (this.image.indexOf("Fan") > -1)
                                        this.cachedValues[Characteristic.RotationSpeed.UUID] = brightness;
                                    else
                        ...
                        if (typeof message.svalue1 == 'undefined') {
                            if (this.image.indexOf("Fan") > -1)
                                this.cachedValues[Characteristic.RotationSpeed.UUID] = 0;
                            else
    ...
    getServices: function () {
        ...
        informationService.setCharacteristic(Characteristic.Manufacturer, "iBeyondSmart").setCharacteristic(Characteristic.Model, this.Type).setCharacteristic(Characteristic.SerialNumber, "Device ID " + this.idx);
        ...
        if (this.isSwitch) {
            ...
            case this.swTypeVal == Constants.DeviceTypeMedia:
                {
                    var lightbulbService = false;
                    if (this.image.indexOf("Fan") > -1)
                    {
                        lightbulbService = this.getService(Service.Fan);
                        if (!lightbulbService)
                        {
                            lightbulbService = new Service.Fan(this.name);
                        }
                        this.getCharacteristic(lightbulbService, Characteristic.On).on('set',this.setPowerState.bind(this)).on('get', this.getPowerState.bind(this));
                        this.gracefullyAddCharacteristic(lightbulbService, Characteristic.RotationSpeed).on('set',this.setdValue.bind(this)).on('get',this.getdValue.bind(this));
                        this.getCharacteristic(lightbulbService, Characteristic.RotationSpeed).setProps({
                            minValue: 0,
                            maxValue: 5,
                            unit: 'l',
                            minStep: 1
                        ...
                        this.getCharacteristic(lightbulbService, Characteristic.On).on('set', this.setPowerState.bind(this)).on('get', this.getPowerState.bind(this));
                        this.gracefullyAddCharacteristic(lightbulbService, Characteristic.Brightness).on('set', this.setdValue.bind(this)).on('get', this.getdValue.bind(this));
                        if (this.subType == "RGBW" || this.subType == "RGBWW" || this.subType == "RGBWWZ") {
                            this.gracefullyAddCharacteristic(lightbulbService, Characteristic.Hue).on('set', this.setHueValue.bind(this, 'Hue')).on('get', this.getHueValue.bind(this, 'Hue'));
                            this.gracefullyAddCharacteristic(lightbulbService, Characteristic.Saturation).on('set', this.setHueValue.bind(this, 'Saturation')).on('get', this.getHueValue.bind(this, 'Saturation'));
                        }
                    }
